...
GR=[1 5 10 20 40 60 80 100 125 150 200 300 500 600];
DX = [10 2 1 0.5 0.2];
for i=1:length(DX)
    delX = DX(i);
    BATCH_NAME = sprintf('DX%.1f_DT0.14_TAU_6_NO_FLYTIME_CORR',delX);
    mkdir(BATCH_NAME);
    for j=1:length(GR)
        NEMBERS_MIN          = GR(j);
        NEMBERS_MAX          = GR(j);
        BATCH_TEST;
        filename=sprintf('DX%.1f_GR%.3f',delX,NEMBERS_MIN);
...
ROS=zeros(1,6);
DEPTH=zeros(1,6);
CASE_NAME = dir('DX10.0_DT0.14_TAU_6_NO_FLYTIME_CORR/DX*');
for i=1:length(CASE_NAME)
    ...
    GR_TEMP = cell2mat(MODEL_INPUTS(3));
...
xlabel('GR [m^{-1}s^{-1}]');ylabel('ROS [m/s]')
set(gca,'FontSize',15)
